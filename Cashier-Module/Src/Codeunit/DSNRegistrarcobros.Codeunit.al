codeunit 51100 "DSNRegistrar cobros"
{
    ...
    begin
        ...
        UserSetup.Reset();
        UserSetup.Get(UserId);
        FuncionesCobros.ValidaCajaAbierta(UserId); //GRN 22/02/2023
        ...
        if MontoCambio <> 0 then begin
            ...
            if not CobrosCajeros.FindFirst() then begin
                ...
                CobrosCajeros2.FindLast(); //GRN Para buscar la ultima linea                
            ...
            if (CobrosCajeros.FindSet()) and (SIH2."Applies-to Doc. No." = '') then
                repeat
                    ...
                    if not ConfMediosdepagos.Credito then begin
                        ...
                        GenJnlLine.VALIDATE("Account No.", ConfMediosdepagos."Account No."); //GRN 20/01/23Tenia Codcaja pero no se llenaba por explicacion de Juan a Agustin
                        ...
                        //GenJnlLine.VALIDATE("Currency Code", SIH2."Currency Code");
                        ...
                        GenJnlLine.VALIDATE("Debit Amount", round(CobrosCajeros."Importe($)", 0.01)); //8-8-2022 jpg usar importe de lo que se coloco en caja
                        ImporteTotalPagado += GenJnlLine."Debit Amount";
                        GenJnlLine."Payment Method Code" := ConfMediosdepagos."Cod. Forma Pago"; //8-8-2022 jpg usar importe de lo que se coloco en caja
            ...
            //GRN Insertado para realizar un solo asiento al auxiliar de clientes
            if ImporteTotalPagado <> 0 then begin
                ...
                GenJnlLine."System-Created Entry" := true;
                //GenJnlLine.VALIDATE("Currency Code", SIH2."Currency Code");
                GenJnlLine.VALIDATE("Credit Amount", ROUND(ImporteTotalPagado, 0.01)); //8-8-2022 jpg usar importe de lo que se coloco en caj
                GenJnlLine."Payment Method Code" := ConfMediosdepagos."Cod. Forma Pago"; //8-8-2022 jpg usar importe de lo que se coloco en caja
                GenJnlLine."Document Type" := GenJnlLine."Document Type"::Payment;
                GenJnlLine."Applies-to Doc. Type" := GenJnlLine."Applies-to Doc. Type"::Invoice;
                GenJnlLine."Applies-to Doc. No." := SIH2."No.";
            ...
            CobrosCajeros2.SetRange("Tipo documento", 1, 2);
            CobrosCajeros2.SetFilter("No. documento", '%1|%2', SIH2."Order No.", SIH2."Pre-Assigned No.");
            //CobrosCajeros2.SETRANGE("Fecha registro",SIH2."Posting Date");
            ...
            CobrosCajeros.SetRange("No. Registro Factura", SIH2."No.");
            //CobrosCajeros.SETFILTER("Forma de pago DGII", '<>%1', CobrosCajeros."Forma de pago DGII"::Efectivo);
            if CobrosCajeros.FindSet() then begin
                ...
                //GRN Para insertar una sola linea en el asiento del cliente
                if ImporteTotalPagado <> 0 then begin
                    GenJnlLine.Init();
                    GenJnlLine."Line No." := NoLin;
                    GenJnlLine."Document No." := SIH2."No.";
                    ...
                    GenJnlLine.Description := COPYSTR(StrSubStNo(Msg004Lbl, SIH2."No." + ', ' + ConfMediosdepagos."Cod. Forma Pago"), 1, MAXSTRLEN(GenJnlLine.Description));
                    GenJnlLine."System-Created Entry" := true;
                    GenJnlLine.VALIDATE("Credit Amount", ROUND(ImporteTotalPagado, 0.01)); //8-8-2022 jpg usar importe de lo que se coloco en caja
                    GenJnlLine."Payment Method Code" := ConfMediosdepagos."Cod. Forma Pago"; //8-8-2022 jpg usar importe de lo que se coloco en caja
                    GenJnlLine."Document Type" := GenJnlLine."Document Type"::Payment;
