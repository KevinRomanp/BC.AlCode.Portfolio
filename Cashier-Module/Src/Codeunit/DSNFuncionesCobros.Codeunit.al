...
"DSNFunciones Cobros"
{
    ...
    local procedure OnRunBeforePreview (Subscriber: Variant; RecVar: Variant)
    var
    SalesHeader : record "Sales Header";
    DSNCobrosCajeros : record "Cobros DSNCajeros";
    begin
    ...
    begin
        ...
        if (SalesHeader."Document Type" <> SalesType::Invoice) or (SalesHeader."Document Type" <> SalesType::Order) then
            exit;
        /*if SalesHeader."Payment Method Code" = 'CRÉDITO' then //TODO: hardcode
            exit;*/
        DSNCobrosCajeros.Reset();
        ...
        if DSNCobrosCajeros.FindFirst() then begin
            ...
            //TotalCobrado(SalesHeader."Document Type", SalesHeader."No.", TotalDocumento, TotalDocumentoLCY);
        end
    end;
