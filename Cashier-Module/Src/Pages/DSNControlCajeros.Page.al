    ...
    actions
    {
        ...
        area(Processing)
        {
            ...
            Action(Cuadre)
            {
                ...
                Image = PrintReport;
                ToolTip = 'Executes the Cuadre de caja action.';
                trigger OnAction()
                var
                    ErrorEstadoLbl: Label 'La caja debe estar cerrada para poder hacer el cuadre.';
                begin
                    CurrPage.SetSelectionFilter(DSNControlCajeros);
                    if rec.EstadoRegistro <> rec.EstadoRegistro::Cerrado then
                        Error(ErrorEstadoLbl);
                    Report.Run(Report::"Cuadre de cajas", true, true, DSNControlCajeros);
                end;
            }
            action(CorregirCaja)
            {
                ApplicationArea = all;
                Caption = 'Corregir Caja';
                trigger OnAction()


                begin
                    Report.Run(report::"Correccion Cobros Cajeros", true);
                end;
            }
            /*action(ya)
            {
                ...
                begin
                    DSNCobrosCajeros.Reset();
                    DSNCobrosCajeros.SetRange(Usuario, 'JCUEVAS');
                    DSNCobrosCajeros.SetRange("No. Documento", 'VP-003283');
                    DSNCobrosCajeros.FindFirst();
                    DSNCobrosCajeros.Delete();
                    /*repeat
                        DSNCobrosCajeros."No. Batch" := -1;
                        DSNCobrosCajeros.Status := DSNCobrosCajeros.Status::Cerrado;
                        DSNCobrosCajeros.Modify;
                    until DSNCobrosCajeros.Next() = 0;
                    SalesInvoiceHeader.Reset();
                    SalesInvoiceHeader.FindSet();
                    repeat
                        SalesInvoiceHeader.BatchNo := -1;
                        SalesInvoiceHeader.Modify();
            ...
            action(aa)
            {
                ...
                begin
                    ...
                    CobrosCajeros2.SetRange("Tipo documento", CobrosCajeros2."Tipo documento"::Order);
                    CobrosCajeros2.SetRange("No. Documento", 'VP-001895');
                    CobrosCajeros2.SetRange("No. Registro Factura", '');
                    //CobrosCajeros2.SetRange(usuario, 'GULF.CSHO');
                    CobrosCajeros2.FindSet();
                    repeat
                        CobrosCajeros2."No. Registro Factura" := 'VFR-001844';
                        //CobrosCajeros2."Cod. Medio de pago" := 'EFECTIVO';
                        //CobrosCajeros2."Forma de pago DGII" := CobrosCajeros2."Forma de pago DGII"::Efectivo;
                        //CobrosCajeros2."ID Agrupacion" := 1;
                        CobrosCajeros2.Modify();
